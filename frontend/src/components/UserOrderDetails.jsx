...
const UserOrderDetails = () => {
  ...
  const refundHandler = async () => {
    await axios.put(`${server}/order/order-refund/${id}`,{
      status: "Повернення коштів у обробці"
    }).then((res) => {
       toast.success(res.data.message);
    dispatch(getAllOrdersOfUser(user._id));
    }).catch((error) => {
  ...
  return (
    <div className={`py-4 min-h-screen ${styles.section}`}>
      ...
      <div className="w-full flex items-center justify-between pt-6">
        ...
        <h5 className="text-[#00000084]">
          Створено: <span>{data?.createdAt?.slice(0, 10)}</span>
        </h5>
      </div>
      ...
      {data &&
        data?.cart.map((item, index) => {
          ...
          <div className="w-full flex items-start mb-5">
            ...
            {!item.isReviewed && data?.status === "Доставлено" ?  <div
        ...
        <div className="w-full fixed top-0 left-0 h-screen bg-[#0005] z-50 flex items-center justify-center">
          <div className="w-[50%] h-min bg-[#fff] shadow rounded-md p-3">
            ...
            <div className="w-full flex">
              ...
              <div>
                ...
                <h4 className="pl-3 text-[20px]">
                  UAH₴{selectedItem?.discountPrice} x {selectedItem?.qty}
                </h4>
              </div>
            </div>
            ...
            <h5 className="pl-3 text-[20px] font-[500]">
              Оцінити <span className="text-red-500">*</span>
            </h5>
      ...
      <br />
      <Link to="/">
        <div className={`${styles.button} text-white`}>Надіслати повідомлення</div>
      </Link>
      <br />
      <br />
    </div>
